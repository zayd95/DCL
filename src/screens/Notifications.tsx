...
export const Notifications = ({ onBack, onNavigate }: { onBack: () => void, onNavigate: (screen: string) => void }) => {
  ...
  return (
    <div className="h-screen bg-[#F8FAFC] max-w-[480px] mx-auto relative flex flex-col font-sans overflow-hidden">
      <header className="bg-ocean-dark p-6 rounded-b-[40px] shadow-xl relative overflow-hidden sticky top-0 z-50 flex-none">
          ...
          {['all', 'critical', 'warning'].map((f) => (
            <button
              ...
              className={cn(
                ...
                filter === f ? "bg-white text-ocean-dark shadow-lg" : "bg-white/10 text-white/50 border border-white/5"
              )}
            >
      ...
      <div className="flex-1 p-6 space-y-4 pb-32 overflow-y-auto no-scrollbar scroll-smooth">
        {loading ? (
          <div className="py-20 text-center text-gray-300">
            <div className="w-10 h-10 border-4 border-gray-100 border-t-ocean-primary rounded-full animate-spin mx-auto mb-4" />
            <p className="text-[10px] font-black uppercase tracking-widest">Récupération des alertes...</p>
          </div>
            ...
            >
              {!alert.read && (
                <div className={cn(
                  ...
                  alert.level === 'critical' ? "bg-red-500" : "bg-ocean-primary"
                )} />
              )}
              ...
              <div className="flex gap-3.5">
                <div className={cn(
                  ...
                  alert.level === 'critical' ? "bg-red-50 text-red-500" : "bg-blue-50 text-ocean-primary"
                )}>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-0.5">
                    <h3 className="text-[13px] font-black text-ocean-dark uppercase tracking-tight leading-tight pr-4">
                      {alert.title}
                    </h3>
                    <p className="text-[8px] font-bold text-gray-300 whitespace-nowrap">
                     ...
                     <div className="flex gap-1.5">
                        {!alert.read && (
                          <button 
                            ...
                            className="bg-green-500 text-white px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest shadow-sm"
                          >
                            Lu
                          </button>
                        )}
                        <button 
                          ...
                          className="bg-gray-50/50 text-gray-400 p-1.5 rounded-lg border border-gray-100/50"
                        >
                          <Trash2 size={10} />
                        </button>
                     </div>
