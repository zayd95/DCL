...
export const DepotManager = ({ onBack, onNavigate }: { onBack: () => void, onNavigate: (screen: string) => void }) => {
  ...
  return (
    <div className="h-screen bg-[#F8FAFC] flex flex-col max-w-[480px] mx-auto relative overflow-hidden font-sans">
      <header className="bg-ocean-dark p-6 rounded-b-[32px] shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-white/50 uppercase tracking-widest mt-1">DEPOTEK HUB</p>
            </div>
          </div>
          <button 
            onClick={() => { setEditingDepot(null); setIsModalOpen(true); }}
            className="w-10 h-10 bg-white text-ocean-primary rounded-xl flex items-center justify-center shadow-lg active:scale-90 transition-all"
          >
            <Plus size={20} />
          </button>
        </div>
      ...
      <div className="p-5 space-y-4 flex-1 overflow-y-auto pb-40">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white/50 backdrop-blur-sm border border-white rounded-xl p-3.5 pl-11 text-xs font-black outline-none focus:ring-4 focus:ring-ocean-primary/5 shadow-sm transition-all placeholder:text-gray-300"
          />
        </div>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="py-20 text-center opacity-20">
              ...
              <p className="text-xs font-black uppercase tracking-widest text-[#0A2540]">Aucun dépôt trouvé</p>
            </div>
          ) : (
      ...
      <AnimatePresence>
        {isDeleteConfirmOpen && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsDeleteConfirmOpen(null)} className="fixed inset-0 bg-[#0A2540]/80 backdrop-blur-sm" />
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-[340px] bg-white rounded-[32px] p-8 z-[310] shadow-2xl space-y-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-black text-[#0A2540] tracking-tight">Supprimer le dépôt ?</h3>
                <p className="text-xs text-red-400 font-bold bg-red-50 p-3 rounded-xl mt-3">ZONE DE DANGER : Cette action est irréversible.</p>
                <p className="text-xs text-gray-400 mt-4 italic">Tapez <span className="font-black text-red-500">SUPPRIMER</span> pour confirmer.</p>
              </div>
              <input 
                value={deleteInput}
              ...
              <div className="flex gap-4">
                <button onClick={() => setIsDeleteConfirmOpen(null)} className="flex-1 py-4 text-gray-400 font-black uppercase text-[10px]">Annuler</button>
                <button 
                  onClick={() => handleDelete(isDeleteConfirmOpen)}
                  disabled={deleteInput !== 'SUPPRIMER'}
                  className={cn(
                    "flex-1 py-4 rounded-2xl font-black uppercase text-[10px] transition-all",
                    deleteInput === 'SUPPRIMER' ? "bg-red-500 text-white shadow-lg shadow-red-500/20" : "bg-gray-100 text-gray-300"
                  )}
                >
                  Supprimer
                </button>
    ...
    <div className="bg-white/70 backdrop-blur-xl p-3.5 rounded-[20px] border border-white/50 shadow-lg shadow-black/5 flex items-center justify-between group active:scale-[0.98] transition-all relative">
      <div className="flex items-center gap-3 flex-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-[11px] font-black text-[#0A2540] uppercase tracking-tight truncate">{depot.name}</h4>
            <span className="px-1.5 py-0.5 bg-white/50 text-gray-400 text-[6px] font-black rounded-md border border-gray-100 uppercase tracking-widest whitespace-nowrap">{depot.code || 'NA'}</span>
          </div>
          <div className="flex items-center gap-2.5 mt-0.5 opacity-60">
            <span className="text-[7px] font-black text-gray-400 uppercase flex items-center gap-0.5"><MapPin size={7} /> CENTRE DEPOTEK</span>
            <span className="text-[7px] font-black text-gray-400 uppercase flex items-center gap-0.5"><Box size={7} /> {depot.capacity_cartons} CTN</span>
          </div>
          <p className={cn(
            "text-[7px] font-black uppercase tracking-widest mt-2 px-2 py-0.5 rounded-full w-fit",
      ...
      <div className="relative flex items-center gap-1.5">
        ...
        <button 
          ...
          className="w-8 h-8 flex items-center justify-center bg-gray-50/50 rounded-lg text-gray-300 hover:text-ocean-primary transition-all active:scale-90"
        >
          ...
          {showMenu && (
            <>
              ...
              <motion.div 
                ...
                className="absolute right-0 top-14 w-44 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-[70] flex flex-col gap-1 overflow-hidden"
              >
...
const MenuAction = ({ icon, label, onClick, danger }: any) => (
  <button 
    onClick={onClick}
    className={cn(
      "flex items-center gap-3 w-full p-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors",
      danger ? "text-red-500 hover:bg-red-50" : "text-[#0A2540] hover:bg-gray-50"
    )}
  >
    {icon} {label}
  </button>
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-end justify-center">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-[#0A2540]/60 backdrop-blur-sm" />
          <motion.div 
          ...
          >
            ...
            <h3 className="text-xl font-black text-[#0A2540] tracking-tight">{editingDepot ? 'Modifier le Dépôt' : 'Nouveau Dépôt'}</h3>
            
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Nom</label>
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="Almadi 3" className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm font-black outline-none focus:ring-4 focus:ring-blue-500/10" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Code</label>
                  <input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="DKR-A3" className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm font-black outline-none" />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Type de Stockage</label>
            ...
            <button 
              ...
              className={cn(
                ...
                type === t ? "bg-ocean-dark text-white shadow-lg" : "bg-gray-50 text-gray-300"
              )}
            >
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Capacité Max (CTN)</label>
                <input type="number" value={capacity} onChange={e => setCapacity(e.target.value)} className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm font-black outline-none" />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-2">Couleur Accent</label>
            ...
            <button onClick={handleSubmit} className="w-full py-5 bg-ocean-primary text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-ocean-primary/20 active:scale-95 transition-all mt-4">
