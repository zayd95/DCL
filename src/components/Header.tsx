...
export const Header = ({ onMicClick, onAddDepot, onInvoiceClick }: { onMicClick: () => void, onAddDepot: () => void, onInvoiceClick: () => void }) => {
  ...
  return (
    <header className="sticky top-0 z-50 pt-6 pb-4 px-4 overflow-hidden relative">
      {/* Glassmorphism Background with Ocean Gradient */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#0A2540] to-[#0066FF] opacity-90 backdrop-blur-xl -z-10" />
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] mt-1">LOGISTICS OS V2</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar -mx-4 px-4 scroll-smooth">
        <button
          ...
          className="flex-none bg-ocean-soft text-ocean-primary px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2 shadow-lg shadow-ocean-primary/20 border border-ocean-primary/10"
        >
          <FileText size={14} />
          Document
        </button>
        ...
        <button
          ...
          className={cn(
            "flex-none px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300",
            !selectedDepotId 
              ? "bg-white text-[#0A2540] shadow-[0_10px_20px_rgba(255,255,255,0.2)]" 
        ...
        {depots.map((depot) => (
          <button
            ...
            className={cn(
              "flex-none px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 relative overflow-hidden",
